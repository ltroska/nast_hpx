...
if len(sys.argv) < 9:
  print("Usage:", sys.argv[0] , "<output grid> <#cols> <#rows> <width> <lower bound for length of rect> <upper bound for length of rect> <lower bound for distance> <upper bound for distance> [max recursion=0('infinite')]", file=sys.stderr)
  sys.exit(0)
...
max_n = sys.maxint
if len(sys.argv) == 10:
	max_n = int(sys.argv[9])

def make_branch(grid, pos, width, distance, n):
	if n <= max_n and not (pos[0] > num_cols - 2 or pos[1] > num_rows - 2):
